...
export class Airdrop implements Contract {
    ...
    async sendDeploy(provider: ContractProvider, via: Sender, value: bigint, nftCollectionWallet: Address) {
        await provider.internal(via, {
            value,
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: beginCell().storeUint(0x610ca46c, 32).storeUint(0, 64).storeAddress(nftCollectionWallet).endCell(),
        });
    }

    async sendChangeNftCollectionnWallet(provider: ContractProvider, via: Sender, value: bigint, nftCollectionWallet: Address) {
        await provider.internal(via, {
            value,
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: beginCell().storeUint(1, 32).storeUint(0, 64).storeAddress(nftCollectionWallet).endCell(),
        });
    }
}
